...
namespace TinkerWorX.AccidentalNoiseLibrary
{
    ...
    {
        private final ThreadLocal<Cache> cache2D = new ThreadLocal<Cache>(() => new Cache());

        private final ThreadLocal<Cache> cache3D = new ThreadLocal<Cache>(() => new Cache());

        private final ThreadLocal<Cache> cache4D = new ThreadLocal<Cache>(() => new Cache());

        private final ThreadLocal<Cache> cache6D = new ThreadLocal<Cache>(() => new Cache());
