...
public class Mapping {
    ...
    public static void Map2D(final MappingMode mappingMode, final double[][] array, final ImplicitModuleBase module,
            final MappingRanges ranges, final double z) {
        ...
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                ...
                case SeamlessXZ:
                    ...
                    dz = ranges.LoopZ1 - ranges.LoopZ0;
                    r = (z - ranges.MapZ0) / (ranges.MapZ1 - ranges.MapZ0);
                    final double xzval = r * (ranges.MapX1 - ranges.MapZ0) / (ranges.LoopZ1 - ranges.LoopZ0);
                    p = p * (ranges.MapX1 - ranges.MapX0) / (ranges.LoopX1 - ranges.LoopX0);
                    nx = ranges.LoopX0 + Math.cos(p * PI2) * dx / PI2;
                    ny = ranges.LoopX0 + Math.sin(p * PI2) * dx / PI2;
                    nz = ranges.MapY0 + q * dy;
                ...
                case SeamlessYZ:
                    ...
                    dz = ranges.LoopZ1 - ranges.LoopZ0;
                    r = (z - ranges.MapZ0) / (ranges.MapZ1 - ranges.MapZ0);
                    final double yzval = r * (ranges.MapZ1 - ranges.MapZ0) / (ranges.LoopZ1 - ranges.LoopZ0);
                    q = q * (ranges.MapY1 - ranges.MapY0) / (ranges.LoopY1 - ranges.LoopY0);
                    nx = ranges.MapX0 + p * dx;
                    ny = ranges.LoopY0 + Math.cos(q * PI2) * dy / PI2;
                    nz = ranges.LoopY0 + Math.sin(q * PI2) * dy / PI2;
                ...
                case SeamlessXYZ:
                    ...
                    q = q * (ranges.MapY1 - ranges.MapY0) / (ranges.LoopY1 - ranges.LoopY0);
                    r = (z - ranges.MapZ0) / (ranges.MapZ1 - ranges.MapZ0);
                    final double xyzval = r * (ranges.MapZ1 - ranges.MapZ0) / (ranges.LoopZ1 - ranges.LoopZ0);
                    nx = ranges.LoopX0 + Math.cos(p * PI2) * dx / PI2;
                    ny = ranges.LoopX0 + Math.sin(p * PI2) * dx / PI2;
                    nz = ranges.LoopY0 + Math.cos(q * PI2) * dy / PI2;
                    nw = ranges.LoopY0 + Math.sin(q * PI2) * dy / PI2;
    ...
    public static void Map2DNoZ(final MappingMode mappingMode, final double[][] array, final ImplicitModuleBase module,
            final MappingRanges ranges) {
        //TODO correct?
        final int width = array.length;
        final int height = array[0].length;
    ...
    public static void Map3D(final MappingMode mappingMode, final double[][][] array, final ImplicitModuleBase module,
            final MappingRanges ranges) {
        //TODO correct?
        final int width = array.length;
        final int height = array[0].length;
